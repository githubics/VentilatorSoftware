...
TEST(Checksum32, KnownValues) {
  EXPECT_EQ((uint32_t)0, crc32(NULL, 0));
  EXPECT_EQ((uint32_t)0, crc32("", 0));
  EXPECT_EQ((uint32_t)0xC808931C, crc32("a", 1));
  EXPECT_EQ((uint32_t)0x47A393F8, crc32("abcde", 5));
  EXPECT_EQ((uint32_t)0x9DBDD91C, crc32("abcdef", 6));
  EXPECT_EQ((uint32_t)0x321FBEF4, crc32("abcdefgh", 8));
}
